...
namespace ecs {
    ...
    void BasicEntityComponentStorage::addComponentSchema(const ComponentSchema &schema) {
    ...
    Vec<Entity> BasicEntityComponentStorage::request(const EntityRequest &request) {
        ...
        std::transform(componentList.begin(), componentList.end(), std::back_inserter(newComponentList), [this](String componentName) -> std::tuple<String, size_t> {
    ...
    ComponentSchema BasicEntityComponentStorage::getSchema(const String &name) const {
        for (auto it = m_schemas.begin(); it != m_schemas.end(); it++) {
            if (it->getName() == name) {
                return *it;
            }
...
std::ostream &operator<<(std::ostream &stream, const ecs::BasicEntityComponentStorage &components) {
    for (auto schema : components.getComponentsSchemas()) {
        stream << schema.getName() << ":" << std::endl;
        for (auto map : components.getComponents(schema.getName())) {
