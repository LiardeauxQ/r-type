...
std::unique_ptr<Message> CollisionTrigger::from(void *data) {
    ...
    auto msg = std::make_unique<CollisionTrigger>(
            pkt->first_entity, pkt->first_type,
            pkt->second_entity, pkt->second_type, pkt->position);
...
std::vector<uint8_t> CollisionTrigger::serialize() const {
    ...
    collision_trigger_t pkt = {m_firstEntity, m_firstType, m_secondEntity, m_secondType, m_position};
