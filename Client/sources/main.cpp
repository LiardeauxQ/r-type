...
namespace animation {
        ...
        , backward(false)
        {
            ...
            for (int i = 0; i + size.y <= tempSize.y; i += size.y) {
                for (int j = 0; j + size.x <= tempSize.x; j += size.x) {
                    rects.push_back(sf::IntRect{j, i, static_cast<int>(size.x), static_cast<int>(size.y)});
                }
            }
            it = rects.begin();
        }
    ...
    public:
        ...
        , backward(false)
        {
            ...
            m_sprite.setPosition({static_cast<float>(windowSize.x), 0});
...
{
    srand((int)time(0));
    TextureFactory<sf::Texture> factory;

    factory.addTexture("ships", "Client/assets/r-typesheet42.gif");
    factory.addTexture("projectiles", "Client/assets/r-typesheet1.gif");
    factory.addTexture("enemies", "Client/assets/r-typesheet11.gif");
    ...
    auto ship1 = animation::Animation(factory.getTexture("ships"), sf::Vector2f(33, 17), 400000, animation::AnimationType::TRIGO, sf::Vector2u(166, 17));
    int ship1Cooldown = 0;
    sf::Vector2f pos1(20, (window.getSize().y / 4));
    auto ship2 = animation::Animation(factory.getTexture("ships"), sf::Vector2f(33, 17), 400000, animation::AnimationType::TRIGO, sf::Vector2u(166, 17));
    int ship2Cooldown = 0;
    ...
    while (window.isOpen()) {
        ...
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::LShift)) {
            if (!ship1Cooldown) {
                projectiles.push_back(std::move(animation::Projectile(factory.getTexture("projectiles"), ship1.getPosition())));
        ...
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::RShift)) {
            if (!ship2Cooldown) {
                projectiles.push_back(std::move(animation::Projectile(factory.getTexture("projectiles"), ship2.getPosition())));
        ...
        if (elapsed < 0) {
            ...
            enemies.push_back(std::move(animation::Enemy(factory.getTexture("enemies"), window, sf::Vector2f(33, 34), 400000, animation::AnimationType::TRIGO, sf::Vector2u(100, 34))));
            enemies.back().setPosition({static_cast<float>(window.getSize().x - (rand() % 100)), static_cast<float>(rand() % window.getSize().y)});
            respawn_time -= 10000;
        }
